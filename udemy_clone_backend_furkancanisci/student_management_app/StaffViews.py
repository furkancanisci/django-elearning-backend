...
from .models import CustomUser, Staffs, Courses, Subjects, Students, SessionYearModel, Attendance, AttendanceReport, LeaveReportStaff, FeedBackStaffs, StudentResult, Lesson
...
def staff_add_lesson_save(request):
    if request.method != "POST":
        ...
        try:
            ...
            lesson.save()
            messages.success(request, "Lesson basarıyla eklendi")
            return redirect('staff_add_lesson')
        except:
            messages.error(request, "Ders eklenemedi!")
            return redirect('staff_add_lesson')
